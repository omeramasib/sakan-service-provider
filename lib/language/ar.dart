const Map<String, String> arSA = {
  // login screen
  'login': 'تسجيل دخول',
  'create account': 'انشاء حساب',
  'register': ' انشاء حساب جديد',
  'skip': 'تخطي',
  'phone_number': 'رقم الجوال',
  'password': 'كلمة المرور',
  'forget_password': 'نسيت كلمة المرور ؟',
  'login_failed': 'خطأ في رقم الهاتف او كلمة المرور',
  'user_not_active':
      'عفوا هذا الحساب غير مفعل يرجى التواصل مع الإدارة لتفعيل الحساب',
  'login_success': 'تم تسجيل الدخول بنجاح',
  'user_not_allowed': 'عفوا هذا الحساب غير مسموح له بالدخول لهذا التطبيق',
  'user_not_exist': 'عفوا هذا الحساب غير موجود يرجى التأكد من البيانات المدخلة',
  'user_not_verified':
      'عفوا هذا الحساب غير مفعل يرجى إدخال كود التفعيل المرسل لك',
  // register screen
  'daklia_name': 'اسم الداخلية',
  'first_name': 'الاسم الاول',
  'last_name': 'اسم العائلة',
  'identity_number': 'رقم الهوية/ الاقامة',
  'igama_number': 'رقم الهوية',
  'id': 'رقم الهوية',
  'phone_empty': 'الرجاء ادخال رقم الجوال',
  'valid_phone': 'يرحى ادخال رقم جوال صحيح',
  'password_validation': 'كلمة المرور يجب ان تتكون من حروف وارقام',
  'required_field': 'الحقل مطلوب',
  'validate_first_name': 'الرجاء ادخال الاسم الاول',
  'valid_first_name': 'الاسم يجب ان يكون اكثر من حرفين',
  'validate_identity_number': 'الرجاء ادخال رقم الهوية',
  'validate_last_name': 'الرجاء ادخال اسم العائلة',
  'valid_identity_number': 'رقم الهوية يجب ان يتكون من 10 ارقام',
  'valid_last_name': 'اسم العائلة يجب ان يكون اكثر من حرفين',
  'validate_confirm_passwords': 'لايوجد تطابق مع كلمة المرور',
  'validate_password': 'الرجاء ادخال كلمة المرور',
  'valid_password': ' كلمة المرور يجب ان تكون على الاقل 8 احرف او ارقام',
  'confirm_password': 'تأكيد كلمة المرور',
  'select_country': 'اختر الدولة',
  'saudi_arabia': 'السعودية',
  'uae': 'الامارات',
  'kuwait': 'الكويت',
  'qatar': 'قطر',
  'bahrain': 'البحرين',
  'oman': 'عمان',
  'sucsses_register': 'تم انشاء الحساب بنجاح',
  'connection_error': 'خطأ في الاتصال بلإنترنت',
  'igama_already_exist': 'رقم الهوية موجود مسبقا',
  'phone_already_exist': 'رقم الجوال موجود مسبقا',
  'user_already_exist': 'اسم الداخلية موجود مسبقا',
  'unknown_error': 'خطأ غير متوقع',
  'no_internet': 'لا يوجد اتصال بالانترنت',
  'server_error': 'خطأ في الخادم',
  'error': 'خطأ',
  'success': 'نجاح',
  'loading': 'جاري التحميل',
  // otp screen
  'wrong_otp': 'كود التحقق غير صحيح',

  // reset password
  'password_cant_be_as_old_one':
      'كلمة المرور الجديدة لا يمكن ان تكون نفس كلمة المرور القديمة',
  'password_changed_successfully': 'تم تغيير كلمة المرور بنجاح',
  // change password
  'old_password_not_correct': 'كلمة المرور القديمة غير صحيحة',

  // logout screen
  "invalid_token.": "الرجاء تسجيل الدخول مرة اخرى",
  // account complete screen
  "daklia_information": "بيانات الداخلية",
  "address": "العنوان",
  "daklia_documentation": "توثيق الداخلية",
  'complete_information': 'اكمال البيانات',
  'complete_information_des':
      'دعنا نتعرف عليك وعلى حالتك الصحية ستساعد\n هذه المعلومات الأطباء في تشخيص حالتك',
  'select_location': 'اختر الموقع',
  'other_detailes': 'تفاصيل اخرى',
  'address_details': 'العنوان',
  'address_details_des': 'صف العنوان بتفصيل اكثر',
  'daklia_documentation_des':
      'ادخل البيانات التي سيتم من خلالها توثيق حساب الداخلية،\nرجاء تأكد من ادخال بيانات صحيحة',
  'daklia_doc': 'صورة من تصديق الداخلية ساري الصلاحية',
  'daklia_owner_doc': 'صورة من إثبات شخصية مالك الداخلية',
  'sucsses_add_daklia_location': 'تم اضافة عنوان الداخلية بنجاح',
  "add_photo": "اضف صورة",
  'sucsses_submit_confirm_account':
      'تم ارسال بيانات توثيق الحساب بنجاح سيتم مراجعته من قبل الادارة وسيتم ارسال رسالة تأكيد اليكم',
  'confirm_account': 'توثيق الحساب',
  'confirm_account_des':
      'سيتم توثيق الحساب بأقرب وقت ممكن حيث\nسيظهر حساب الداخلية لمستخدمين التطبيق\n في قائمة الداخليات المتاحة',
  'daklia_page': 'صفحة الداخلية',
  'gallery': 'المعرض',
  'camera': 'الكاميرا',
  '1-from-4': '1 من 4  بيانات عامة',
  '2-from-4': '2 من 4  اكمال الملف الطبي',
  '3-from-4': '3 من 4  اكمال الملف الطبي',
  '4-from-4': '4 من 4  اكمال الملف الطبي',
  'height': 'الطول',
  'width': 'الوزن',
  '80': '80',
  '180': '180',
  'family_member_number': 'رقم جوال احد افراد العائلة',
  'add_another_number': 'اضافة رقم اخر',
  'next': 'التالي',
  'ignore': 'تجاهل',
  'other_details': 'تفاصيل أخرى',
  'det': 'تفاصيل',
  'obstruction_question': 'هل لديك إعاقة ؟',
  'obstruction_name': 'مااسم الإعاقة؟',
  'warning':
      'ستظهر هذه البيانات في الملف التعريفي للداخلية\nتأكد من كتابتها بشكل جيد وإضافة صور ذات جودة عالية',
  'daklia_image': 'صورة الداخلية',
  'daklia_description': 'وصف عن الداخلية',
  'room_count': 'عدد الغرف',
  'user_id_already_exist': 'عفوا لقد قمت بملئ هذه البيانات من قبل',
  'daklia_image_required': 'صورة الداخلية مطلوبة',
  'daklia_description_required': 'وصف عن الداخلية مطلوب',
  'token_is_invalid': 'الرجاء تسجيل الدخول مرة اخرى',
  'please_select_image': 'الرجاء اختيار صورة للداخلية من المعرض او الكاميرا',
  'please_select_daklia_doc':
      'الرجاء اختيار صورة من تصديق الداخلية ساري الصلاحية',
  'please_select_daklia_owner_doc':
      'الرجاء اختيار صورة من إثبات شخصية مالك الداخلية',
  'daklia_dosent_exist':
      'الداخلية غير موجودة يرجى تسجيل الدخول او التسجيل مرة اخرى',
  'please_select_room_image': 'الرجاء اختيار صورة للغرفة من المعرض او الكاميرا',
  'Failed_to_remove_room': 'فشل في حذف الغرفة',
  'room_not_exist': 'الغرفة غير موجودة',
  'delete_room_successufully': 'تم حذف الغرفة بنجاح',
  // Change Location Screen
  'success_change_location': 'تم تغيير عنوان الداخلية بنجاح',
  'invalid_data': 'الرجاء ادخال بيانات صحيحة',
  // otp screen
  'enter_otp': 'ادخل كود التحقق',
  'description1_otp': 'ارسلنا كود التحقق في رسالة على رقم جوالك',
  'description2_otp': 'قم بإدخاله هنا من فضلك',
  'not_receive_otp': 'لم يصل لك كود ؟',
  'resend_otp': 'اضغط هنا',
  'confirm': 'تأكيد',
  'previous': 'السابق',
  'choose_habits': 'اختر العادات التي تمارسها',
  'smoking': 'التدخين',
  'drinking': 'تعاطي الكحول',
  'take_drugs': 'تناول المخدرات',
  'nothing': 'لا شيء',
  'genetic_diseases': 'هل يوجد لديك مرض وراثي في العائلة؟',
  'disease name': 'اسم المرض',
  'medicament name': 'اسم الدواء',
  'not_receive_code': 'لم يصلك كود التحقق ؟',
  'resend': 'اعادة ارسال',
  'validate_otp': 'يرجى ادخال كود التحقق',
  'valid_otp': 'رقم التحقق يجب ان يكون 6 ارقام',
  // forget password screen
  'phone_number_does_not_exist': ' عفوا رقم الجوال غير موجود',
  'forget_password_description':
      ' رجاء ادخل رقم الجوال الذي تم تسجيله معنا لإعادة تعيين كلمة المرور',
  // reset password screen
  'reset_password': 'إعادة تعيين كلمة المرور',
  'save': 'حفظ',
  'new_password': 'كلمة المرور الجديدة',
  'confirm_new_password': 'تأكيد كلمة المرور الجديدة',
  // home screen
  'see_all': 'رؤية الكل',
  'home': 'الرئيسية',
  'profile': 'البروفايل',
  'more': 'المزيد',
  // appointment screen
  'my_appointments': 'حجوزاتي',
  "status": "الحالة",
  'new_bookings': 'حجوزات جديدة',
  'previous_bookings': 'حجوزات سابقة',
  'awaiting_daklia_approval': 'بإنتظار موافقتك',
  'awaiting_user_approval': 'بإنتظار موافقة الطالبة',
  "booking_number": "رقم الحجز",
  'payment_informaton': 'معلومات الدفع',
  'booking_type': 'نوع الحجز',
  'canceld': 'ملغي',
  'completed': 'مكتمل',
  'pending': 'قيد التنفيذ',
  'booking_details': 'بيانات الحجز',
  'student_details': 'بيانات الطالبة',
  'paid': 'مدفوع',
  'no_pending_bookings': 'عفوا لاتوجد حجوزات بعد',
  'beds_number_in_room': 'عدد السرائر في الغرفة',
  'room_price': 'سعر الغرفة',
  'accept': 'قبول',
  'reject': 'رفض',
  'main_info': 'البيانات الأساسية',
  'guardian_id': 'رقم هوية ولي الأمر',
  'university/workplace': 'اسم الجامعة/ مكان العمل',
  'ID_photo': 'صورة اثبات الشخصية',
  'appointment_rejection_reason': 'وضح سبب الرفض',
  'appointment_reject': 'رفض الحجز',
  'no_previous_bookings': 'عفوا لاتوجد حجوزات سابقة',
  'send': 'ارسال',

  // Booking status and currency
  'sdg': 'ج.س',
  'approved': 'موافق عليه',
  'rejected': 'مرفوض',
  'cancelled': 'ملغي',
  'daily': 'يومي',
  'monthly': 'شهري',
  'customer_name': 'اسم العميل',
  'customer_phone': 'رقم العميل',
  'customer_type': 'نوع العميل',
  'employee': 'موظف',
  'student': 'طالب',
  'dates': 'التواريخ',
  'beds_booked': 'عدد السرائر المحجوزة',
  'booking_approved': 'تمت الموافقة على الحجز بنجاح',
  'booking_rejected': 'تم رفض الحجز',
  'cancel_booking': 'إلغاء الحجز',
  'booking_cancelled': 'تم إلغاء الحجز بنجاح',
  'cancel_booking_confirm': 'هل أنت متأكد من إلغاء هذا الموعد؟',
  'please_enter_rejection_reason': 'الرجاء ادخال سبب الرفض',
  'rejection_reason': 'سبب الرفض',
  'unknown': 'غير معروف',
  'no_booking_data': 'لا توجد بيانات حجز',

  // profile screen
  'confirmed': 'موثق',
  'daklia_description_description':
      'وصف عن الداخلية هذا الوصف الذي ادخله المستخدم \nفي خطوة استبداله حسب طول النص الذي ادخله المستخدم',
  'Save_changes': 'حفظ التغييرات',
  'my_profile': 'الملف الشخصي',
  'medical_profile': 'الملف الطبي',
  'name': 'محمد بن تركي',
  'my_bookings': 'حجوزاتي(16)',
  'next_bookings': 'قادمة',
  'birth_date': 'تاريخ الميلاد',
  'social_status': 'الحالة الإجتماعية',
  'gender': 'الجنس',
  'daklia_management': 'إدارة الداخلية',
  'rooms_management': 'إدارة الغرف',
  'services_management': 'إدارة الخدمات',
  'appointments_management': 'إدارة الحجوزات',
  'settings': 'الإعدادات',
  'terms': 'إدارة اللوائح',
  'room': 'الغرف',
  'services': 'الخدمات',
  'conditions': 'اللوائح',
  'under_documentation': 'قيد التوثيق',
  'un_verified': 'غير موثق',
  "error_occured": "حدث خطأ ما",
  'try_again': 'حاول مرة أخرى',
  // update profile screen
  "success_update_profile": "تم تحديث الملف الشخصي بنجاح",
  'please_select_at_least_one_field':
      'يرجى اختيار حقل واحد على الأقل لتعديل الملف الشخصي',
  // regulation management screen
  'empty_regulation': 'لم تضف أي لوائح بعد',
  'empyt_regulation_des':
      'قم بإضافة قائمة اللوائح الخاصة بالداخلية هنا حتى\n                       تظهر للمستخدمين',
  'add_regulation': 'إضافة لائحة',
  'add_new_regulation': 'إضافة لائحة جديدة',
  'regulation_text': 'نص اللائحة',
  'regulation_des': 'العقوبات المترتبة على مخالفة اللائحة',
  'regulation1': 'يتم اغلاق الداخلية عند الساعة العاشرة م',
  'regulation1_des':
      'هذا النص هو مثال لنص يمكن أن يستبدل في نفس المساحة،لقد تم توليد هذا النص من مولد النص العربي',
  'edit_regulation': 'تعديل بيانات اللائحة',
  'delete_regulation': 'حذف اللائحة',
  'delete_regulation_finally': 'حذف اللائحة نهائيا',
  'delete_regulation_des':
      '         هل انت متأكد من حذف اللائحة\nسيتم حذف جميع البيانات الخاصة بهذه اللائحة',
  // medical_profile screen
  'Q1': 'هل سبق وان زرت طبيب شخصي من قبل ؟',
  'Q2': 'هل يوجد لديك إعاقة؟',
  'Q3': 'هل تعاني من مشاكل صحية حاليا؟',
  'Q4': 'هل تتناول ادوية في الفترة الحالية؟',
  'yes': 'نعم',
  'no': 'لا',
  // edit_profile screen
  'edit_profile': 'تعديل البيانات',
  'warning_edit_profile':
      'رجاء تأكد من ادخال بيانات صحيحة حيث ستظهر هذه البيانات في حساب الداخلية المعروض للمستخدمين',
  'single': 'أعزب/عزباء',
  'married': 'متزوج/ ة',
  'divorced': 'مطلق/ ة',
  'widow': 'أرمل/ ة',
  'sex': 'الجنس',
  'male': 'ذكر',
  'female': 'انثى',
  'save_changes': 'حفظ التعديلات',
  // more screen
  'my_informations': 'بياناتي',
  'account_settings': 'اعدادات الحساب',
  'favorite_doctors': 'قائمة الأطباء المفضلين',
  'connect_with_support': 'تواصل مع الدعم',
  'conditions_and_terms': 'الشروط والاحكام',
  'privacy_policy': 'سياسة الخصوصية',
  'rate_app': 'تقييم التطبيق',
  'about_sanedny': 'عن ساندني',
  'logout': 'تسجيل الخروج',
  'delete_account': 'حذف الحساب',
  'language': 'اللغة',
  // notifications screen
  'notifications': 'الإشعارات',
  'reminder': 'تذكير باقتراب موعد الاستشارة',
  'reply': 'رد على تزكرة تأجيل موعد',
  'new_discounts': 'خصومات جديدة',
  'no_notification_yet': 'لايوجد اي إشعارات بعد',
  'just_now': 'الآن',
  'minutes_ago': 'دقيقة مضت',
  'hours_ago': 'ساعة مضت',
  'days_ago': 'يوم مضى',
  // room management screen
  'no_rooms': 'لايوجد غرف بعد',
  'no_rooms_des':
      '       لم تقم بإدخال اي غرف بعد\nقم بإضافة الغرف الخاصة بك من هنا',
  'add_room': 'إضافة غرفة',
  'rooms': 'الغرف',
  'all_rooms': 'كل الغرف',
  'available_rooms': 'الشاغرة',
  'occupied_rooms': 'المحجوزة',
  'occupied': 'محجوزة',
  'room_number': 'رقم الغرفة',
  'room_type': 'نوع الغرفة',
  'multi_room': ': غرفة متعددة',
  'beds_number': 'عدد السرائر',
  'empty': 'شاغر',
  'single_room': 'غرفة فردية',
  'shared_room': 'غرفة مشتركة',
  'edit_room_info': 'تعديل بيانات الغرفة',
  'delete_room': 'حذف الغرفة',
  'delete_room_finally': 'حذف الغرفة نهائيا',
  'delete_room_des':
      '         هل انت متأكد من حذف الغرفة\nسيتم حذف جميع البيانات الخاصة بهذه الغرفة',
  'add_room_features': 'إضافة مميزات الغرفة',
  'feature': 'الميزة',
  'feature_des': 'وصف الميزة',
  'remove': 'حذف',
  'feature1': 'تتميز الغرفة بإطلالة جميلة',
  'feature2': 'حمام داخلي',
  'otherDetails': 'تفاصيل أخرى',
  'edit_feature': 'تعديل الميزة',
  'delete_feature': 'حذف الميزة',
  'delete_feature_finaly': 'حذف الميزة نهائيا',
  'delete_feature_des':
      '         هل انت متأكد من حذف الميزة\nسيتم حذف جميع البيانات الخاصة بهذه الميزة',
  'edit_information': 'تعديل البيانات',
  'Failed_to_load_rooms': 'فشل في تحميل الغرف',
  'room_added_successfully': 'تم إضافة الغرفة بنجاح',
  'account_not_verified': 'لم يتم تفعيل الحساب بعد يرجى التواصل مع الإدارة',
  'Failed_to_add_room': 'فشل في إضافة الغرفة',
  'image_too_large': 'حجم الصورة كبير جداً، يرجى اختيار صورة أصغر',
  'edit_multiple_room_info': 'تعديل بيانات الغرفة المشتركة',
  'edit_single_room_info': 'تعديل بيانات الغرفة الفردية',
  'Failed_to_update_multiple_room': 'فشل في تعديل بيانات الغرفة المشتركة',
  "success_update_room_information": "تم تعديل بيانات الغرفة بنجاح",
  'daklia_not_exist': 'عفوا الداخلية غير موجودة يرجى التواصل مع الإدارة',
  'edit_room_features': 'تعديل مميزات الغرفة',
  'Failed_to_load_features': 'فشل في تحميل مميزات الغرفة',
  // add Feature
  'feature_added_successfully': 'تم إضافة ميزة الغرفة بنجاح',
  'room_not_belong_to_daklia': 'عفوا هذه الغرفة لا تنتمي للداخلية',
  'room_does_not_exist': 'عفوا هذه الغرفة غير موجودة',
  'Failed_to_add_feature': 'فشل في إضافة ميزة الغرفة',
  // Edit Feature
  'edit_room_feature': 'تعديل مميزات الغرفة',
  'feature_updated_successfully': 'تم تعديل ميزة الغرفة بنجاح',
  'Failed_to_update_feature': 'فشل في تعديل ميزة الغرفة',
  // Delete Feature
  'feature_deleted_successfully': 'تم حذف ميزة الغرفة بنجاح',
  // Add Multi Room
  'add_multi_room': 'إضافة غرفة مشتركة',
  'room_detailes': 'تفاصيل الغرفة',
  'room_features': 'مميزات الغرفة',
  'all_beds_number': ' عدد السرائر الكلي',
  'empty_beds_number': 'عدد السرائر الشاغرة',
  'daily_bed_price': 'سعر السرير بلإستضافة',
  'monthly_bed_price': 'سعر السرير بالشهر',
  'bed_price': 'سعر السرير',
  'room_image': 'صور الغرفة',
  'booking_type_available': 'نوع الحجز المتاح',
  'daily_booking': 'حجز إستضافة',
  'monthly_booking': 'حجز شهري',
  // Add Single Room
  'add_single_room': 'إضافة غرفة فردية',
  'empty_room': 'الغرفة شاغرة',
  'add_feature': 'إضافة ميزة',
  'no_features': 'لايوجد ميزات بعد',
  // services management
  'all_services': 'كل الخدمات',
  'active_services': 'مفعلة',
  'inactive_services': 'غير مفعلة',
  'no_services': 'لا يوجد خدمات بعد',
  'no_services_des':
      '       لم تقم بإضافة اي خدمة بعد\nقم بإضافة الخدمات التي توفرها من هنا',
  'add_service': 'إضافة خدمة',
  'the_services': 'الخدمات',
  'internet_service': 'خدمة الإنترنت',
  'add_new_service': 'إضافة خدمة جديدة',
  'service_name': 'اسم الخدمة',
  'service_price': 'سعر الخدمة',
  'service_active_right_now': 'الخدمة مفعلة حاليا',
  'service_added_successfully': 'تم إضافة الخدمة بنجاح',
  'Failed_to_add_service': 'فشل في إضافة الخدمة',
  'service_not_belong_to_daklia': 'عفوا هذه الخدمة لا تنتمي للداخلية',
  'service_does_not_exist': 'عفوا هذه الخدمة غير موجودة',
  'Failed_to_update_service': 'فشل في تعديل الخدمة',
  'service_updated_successfully': 'تم تعديل الخدمة بنجاح',
  'service_deleted_successfully': 'تم حذف الخدمة بنجاح',
  'Failed_to_load_services': 'فشل في تحميل الخدمات',
  'service_description': 'وصف الخدمة',
  'edit_service': 'تعديل الخدمة',
  'delete_service': 'حذف الخدمة',
  'service_edited_successfully': 'تم تعديل الخدمة بنجاح',
  'Failed_to_edit_service': 'فشل في تعديل الخدمة',
  'Failed_to_remove_service': 'فشل في حذف الخدمة',
  // Laws management
  'law_added_successfully': 'تم إضافة اللائحة بنجاح',
  'Failed_to_add_law': 'فشل في إضافة اللائحة',
  'law_edited_successfully': 'تم تعديل اللائحة بنجاح',
  'law_not_belong_to_daklia': ' عفوا هذه اللائحة لاتنتمي لهذه الداخلية',
  'law_does_not_exist': 'عفوا هذه اللائحة غير موجودة',
  'laws_deleted_successfully': 'تم حذف اللائحة بنجاح',
  'Failed_to_load_laws': 'فشل في تحميل اللوائح',
  // Sanedny screen
  'dr_mohamed': ' محمد أحمد',
  'psychiatry': 'الطب الشخصي',
  'dr_mohamed_description':
      'دكتور محمد دكتور متخصص في الطب الشخصي ولديه 10 سنوات من الخبرة ',
  'book_appointment': 'حجز موعد',
  'about_doctor': 'عن الطبيب',
  'doctor_ratings': 'التقييمات (10)',
  'about_me': 'نبذة عني',
  'doctor_description':
      'دكتور محمد أحمد طبيب متخصص في الطب الشخصي لدي خبرة اكثر من 10 سنوات ورائد في مجال الطب الشخصي اعمل في عيادة ساندني منذ 5 سنوات',
  'years_of_experience': ' عدد سنوات الخبرة',
  '5-years': '10-5 سنوات',
  'skills_and_experies': 'الخبرات والمهارات',
  'since-2012 to 2014': '10/2014 - 08/2012',
  'program': 'تقديم برنامج صحتك الشخصية بتهمنا في قناة العربية',
  'since-2000 to 2006': '10/2006 - 08/2000',
  'program2': 'عملت في مستشفى الشفاء في قسم الطب الشخصي لمدة 6 سنوات متتالية',
  'program3': 'المشاركة في مؤتمر الصحة الشخصية',
  'my_languages': 'لغات اتحدث بها',
  'arabic': 'العربية',
  'english': 'الانجليزية',
  // doctor rating screen
  'ali': 'علي',
  'manar': 'منار',
  'saturday': 'السبت 22 مارس',
  'ali_rating':
      'أسلوب الدكتور محمد رائع جدا سمع لي واعطاني نصائح جعلتني قوي جدا سأكرر التجربة مرة أخرى وانصح الجميع بلدكتور محمد',
  'manar_rating': 'تجربة أكثر من رائعة',
  // appointment screen
  'wait': 'قيد الانتظار',
  'manar_mohammed': 'منار محمد',
  'abdalah_ali': 'عبدالله علي',
  'remain_3': 'باقي 3 ايام',
  'period': 'المدة',
  'appointment': 'الموعد',
  'natural': 'طبيعته',
  '30-min': '30 دقيقة',
  '9 : 00 am': '9:00 ص - السبت 22 مارس',
  'video-consultation': 'استشارة فيديو',
  'since_month': 'منذ شهر',
  'since_year': 'منذ سنة',
  'price': 'السعر',
  '80-SAR': '80 ر.س',
  'cancelle_appointment_booking': 'إلغاء الحجز',
  'booking_management': 'إدارة الحجز',
  'cancel_appointment': 'إلغاء الموعد',
  'cancel_appointment_message':
      'يؤسفنا انك ستلغي الموعد يرجى العلم انه سيتم خصم .. من اجمالي المبلغ المدفوع في حالة الغاء الحجز',
  'to-retreat': 'تراجع',
  'start_consultation': 'بدء الاستشارة',
  'not_consultation_time': 'لم يأتي موعد الاستشارة بعد',
  'consultation_date': 'موعد الاستشارة مع دكتور منال هو',
  'consultation_time_description':
      'الرجاء الانتظار حتى قدوم الموعد ان كان هناك امر طارئ يمكنك طلب استشارة فورية من هنا او التواصل من خلال أرقام الطوارئ',
  'immediate_consultation': 'استشارة فورية',
  'booking_new_appointment': 'حجز موعد جديد مع نفس الطبيب',
  'view_prescription': 'عرض الوصفة الطبية',
  // booking details screen
  'apoointment_details': 'تفاصيل الموعد',
  'prescription': 'الوصفة الطبية',
  'patient_data': 'بيانات المريض',
  'problem': 'المشكلة التي تعاني منها',
  'problem_description':
      'أريد الاستشارة في أمر ضروري أواجه مشاكل اسرية كثيرة ولا اعرف ما الحل المناسب للأمر',
  'details': 'تفاصيل الموعد',
  'attachments': 'المرفقات',
  'Doctor_instructions': 'تعليمات الطبيب',
  'doctor_instructions_description':
      'شعورك بلراحة في مظهرك وطريقة جلوسك مهم حتى لايشتت تفكيرك بأشياء آخرى شعورك بلراحة في مظهرك',
  'medicines': 'الأدوية',
  'validity': 'صلاحية هذه الإستشارة',
  '27/3/2022': '27/3/2022',
  // rating doctor screen
  'rating_question': 'كيف كانت تجربتك مع دكتور محمد؟',
  'write_review': 'اكتب تعليق',
  'your_review': 'تعليقك',
  'add_rating': 'إضافة التقييم',
  'not_yet_filled': 'لم تملئ بعد',
  'not_completed_prescription_description':
      'سعيدين باتمام موعدك بنجاح لم يتم كتابة الوصفة الطبية من قبل الطبيب بعد يرجى الإنتظار حتى يكتب الطبيب الوصفة وسيتم ارسال تنبيه لك عند كتابة الوصفة العلاجية من قبل الطبيب',
  // Search Details Screen
  'search_details': 'نتائج البحث',
  'search': 'بحث',
  'filter_by': 'تصفية حسب',
  'clinic': 'العيادة',
  'consultation_price': 'سعر الاستشارة',
  '50-99': '50-99 ر.س',
  '100-199': '100-199 ر.س',
  '200-300': '200-300 ر.س',
  'more_than_300': 'أكثر من ذلك',
  '0-2': '2-0 سنوات',
  '3-5': '5-3 سنوات',
  '6-10': '10-6 سنوات',
  'more_than_10': 'أكثر من ذلك',
  'doctor_gender': 'جنس الطبيب',
  'rating-percentage': 'نسبة التقييم',
  'List-filter': 'تصفية القائمة',
  // appointment booking screen
  'appointment_booking_des':
      'اختر الوقت المناسب لك واكمل البيانات اللازمة\n لحجز الموعد',
  'january': 'يناير',
  'february': 'فبراير',
  'march': 'مارس',
  'april': 'ابريل',
  'may': 'مايو',
  'june': 'يونيو',
  'july': 'يوليو',
  'august': 'اغسطس',
  'september': 'سبتمبر',
  'october': 'اكتوبر',
  'november': 'نوفمبر',
  'december': 'ديسمبر',
  'good_time': 'الوقت المناسب',
  '9:00': ' 9:00 ص -  9:30 ص',
  'patient_information': 'معلومات عن المريض',
  'who_is_patient': 'من المريض؟',
  'you': 'انت',
  'add_patient': 'اضافة مريض',
  'attach_file': 'ارفق صور او ملفات قد تفيد الطبيب',
  'attach': 'ارفق ملفات',
  'add_another_patient': 'اضافة مريض اخر',
  'relative-relation': 'صلة القرابة',
  'brother_sister': 'أخ /أخت',
  'father_mother': 'أب /أم',
  'son_daughter': 'ابن /ابنة',
  'other': 'غير ذلك',
  'add_button': 'اضافة',
  // payment screen
  'payment': 'الدفع',
  'deposit': 'العربون',
  'discount': 'الخصم',
  'payment_details': 'تفاصيل',
  'add_coupon': 'إضافة كوبون',
  'cost': 'التكلفة',
  '150': '150 ريال',
  'tax': 'الضريبة',
  '10': '10.0 ريال',
  'payment_discount': 'الخصم',
  '0 SAR': '0 ريال',
  'total_amount_payment': 'المبلغ الإجمالي',
  'choose_payment_method': 'اختر طريقة الدفع المناسبة لك',
  'credit_card': 'البطاقات الإئتمانية',
  'sadad': 'سداد',
  'apple_pay': 'Apple Pay',
  'pay': 'دفع',
  'failed_to_initiate_payment': 'فشل بدء الدفع',
  // change password screen
  'change_password': 'تغيير كلمة المرور',
  'old_password': 'كلمة المرور القديمة',
  // Delete Account Screen
  'delete_account_finally': 'حذف الحساب نهائيا',
  'delete_account_finally_des':
      'يؤسفنا انك تريد حذف حسابك بلرجاء ان\n واجهت أي مشكلة او هناك أي شيء ازعجك\n قدم شكوى لنا وسنقوم بحل الأمر',
  'speak_with_support': 'تحدث مع الدعم',
  // Favorite Doctor Screen
  'favorite_doctor': 'قائمة الأطباء المفضلين',
  // Add Ticket Screen
  'add_ticket': 'إضافة تذكرة جديدة',
  'ticket_title': 'عنوان التذكرة',
  'ticket_type': 'نوع التذكرة',
  'questions': 'أسئلة',
  'complaint': 'شكوى',
  'Importance': 'الأهمية',
  'high': 'عالية',
  'medium': 'متوسطة',
  'low': 'منخفضة',
  'notes': 'الملاحظات',
  'add_ticket_button': 'إضافة',
  'ticket_added_successfully': 'تم إضافة التذكرة بنجاح',
  'ticket_added_successfully_des':
      'تم إضافة التذكرة بنجاح سيتم الرد عليك في\n                             أقرب وقت',
  // Doctors Screen
  'doctors': 'الأطباء',
  'all': 'الكل',

  // Room validation messages
  'room_number_required': 'رقم الغرفة مطلوب',
  'room_number_invalid': 'يرجى إدخال رقم غرفة صحيح',
  'beds_number_required': 'عدد السرائر مطلوب',
  'beds_number_invalid': 'يرجى إدخال رقم صحيح',
  'available_beds_required': 'عدد السرائر المتاحة مطلوب',
  'available_beds_invalid': 'يرجى إدخال رقم صحيح',
  'price_required': 'السعر مطلوب',
  'price_invalid': 'يرجى إدخال رقم صحيح',
  'select_room_type': 'يرجى اختيار نوع الغرفة',
  'select_booking_type': 'يرجى اختيار نوع حجز واحد على الأقل',
  'enter_daily_price': 'يرجى إدخال السعر اليومي',
  'enter_monthly_price': 'يرجى إدخال السعر الشهري',

  // Multiple image selection
  'images_selected': 'صور مختارة',
  'select_images': 'اختر الصور',

  // Splash screen
  'under_maintenance': 'تحت الصيانة',
  'retry': 'إعادة المحاولة',
  'update_required': 'يجب تحديث التطبيق',
  'update_message': 'يتوفر إصدار جديد من التطبيق. يرجى التحديث للمتابعة.',
  'update_now': 'تحديث الآن',
  'new_version': 'الإصدار الجديد',
  'whatsapp_not_available': 'تطبيق الواتس اب غير مثبت',
  'error_opening_whatsapp': 'تعذر فتح الواتس اب',

  // Subscription management
  'subscription_management': 'إدارة الاشتراكات',
  'subscription_plans': 'باقات الاشتراك',
  'subscribe': 'اشترك',
  'subscription_status': 'حالة الاشتراك',
  'active_subscription': 'اشتراك نشط',
  'no_active_subscription': 'لا يوجد اشتراك نشط',
  'subscribe_to_continue': 'اشترك للمتابعة',
  'subscription_required': 'اشتراك مطلوب',
  'subscription_required_message':
      'تحتاج إلى اشتراك نشط لاستقبال الحجوزات. الرجاء الاشتراك للمتابعة.',
  'days_remaining': 'الأيام المتبقية',
  'expires_on': 'ينتهي في',
  'subscription_history': 'سجل الاشتراكات',
  'plan_duration': 'المدة',
  'plan_price': 'السعر',
  'payment_processing': 'معالجة الدفع',
  'payment_pending_message': 'يتم معالجة دفعتك. يرجى الانتظار لحظة.',
  'check_again': 'تحقق مرة أخرى',
  'payment_success': 'تم الدفع بنجاح',
  'payment_failed': 'فشل الدفع',
  'payment_cancelled': 'تم إلغاء الدفع',
  'complete_payment': 'إكمال الدفع',
  'subscription_activated': 'تم تفعيل الاشتراك بنجاح!',
  'days': 'يوم',
  'no_plans_available': 'لا توجد باقات نشطة متاحة حاليا',
  'cancel': 'إلغاء',
  'payment_status_check': 'فحص حالة الدفع',
  'payment_taking_time':
      'الدفع يستغرق وقتاً أطول من المتوقع. هل تريد الفحص مرة أخرى؟',
  'close': 'إغلاق',
  'checking': 'جاري الفحص',
  'payment_still_processing': 'لا يزال الدفع قيد المعالجة. يرجى الانتظار.',
  'check_failed': 'فشل فحص حالة الدفع',
  'check_payment_status': 'فحص حالة الدفع',
  'checking_payment_status': 'جاري فحص حالة الدفع...',

  // CashiPay Payment
  'reference_number': 'رقم المرجع',
  'copy': 'نسخ',
  'copied_to_clipboard': 'تم النسخ',
  'or_scan_qr_code': '── أو امسح رمز QR ──',
  'how_to_pay': '── كيفية الدفع ──',
  'option_1_mycashi_app': 'الخيار الاول: تطبيق Cashi',
  'option_2_cashi_merchant': 'الخيار الثاني: وكيل Cashi',
  'mycashi_step_1': 'افتح تطبيق كاشي',
  'mycashi_step_2': 'اضغط على "CashiPay" في الخدمات',
  'mycashi_step_3': 'أدخل رقم المرجع أو امسح رمز QR',
  'mycashi_step_4': 'قم بتأكيد مبلغ الدفع',
  'merchant_step_1': 'قم بزيارة أي متجر يحمل شعار Cashi',
  'merchant_step_2': 'أعط الكاشير رقم المرجع الخاص بك',
  'merchant_step_3': 'ادفع المبلغ نقداً',
  'ive_completed_payment': 'لقد أكملت الدفع',
  'waiting_for_confirmation': 'في انتظار تأكيد الدفع...',
  'payment_timeout': 'الدفع يستغرق وقتاً أطول من المتوقع',
  'payment_timeout_message': 'سيتم تفعيل اشتراكك بمجرد التأكيد',
  'subscription_activated_success': 'تم تفعيل الاشتراك بنجاح',
  'go_to_home': 'الذهاب للرئيسية',
  'payment_enter_otp': 'أدخل رمز التحقق',
  'confirm_otp': 'تأكيد رمز التحقق',
  'expires_in': 'ينتهي في',
  'amount': 'المبلغ',
  'expiring_soon_banner': 'اشتراكك على وشك الانتهاء. قم بالتجديد الآن.',

  // CashiPay OTP Flow
  'reference_and_qr': 'رقم المرجع و QR',
  'pay_with_otp': 'الدفع برمز التحقق (OTP)',
  'enter_wallet_number': 'أدخل رقم محفظة Cashi',
  'send_otp': 'إرسال رمز التحقق',
  'cashi_otp_step_1': '١. قم بإدخال رقم محفظة Cashi الخاصة بك.',
  'cashi_otp_step_2': '٢. ستستلم رمز تحقق (OTP) عبر رسالة SMS.',
  'cashi_otp_step_3': '٣. قم بإدخال رمز التحقق (OTP) المكون من 6 أرقام.',
  'cashi_otp_step_4': '٤. بعد إدخال الرمز بشكل صحيح، سيتم إتمام عملية الدفع بنجاح.',
  'failed_to_confirm_otp': 'فشل تأكيد رمز التحقق (OTP). يرجى المحاولة مرة أخرى.',

  // Network error messages
  'network_error': 'خطأ في الشبكة',
  'network_error_title': 'مشكلة في الاتصال',
  'network_error_message':
      'تعذر الاتصال بالخادم. يرجى التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى.',
  'check_connection': 'تحقق من اتصالك بالإنترنت',
};
